...
export default class SearchDoc extends React.Component<
  ISearchDocProps,
  ISearchDocState,
  {}
> {
  ...
  public viewfields: string =
    '<ViewFields><FieldRef Name="Name"/><FieldRef Name="LSB_AreaOfLaw"/><FieldRef Name="LSB_AreaOfLaw2"/><FieldRef Name="LSB_TopicsOfLaw"/><FieldRef Name="LSB_TopicsOfLaw2"/><FieldRef Name="LSB_Legislation"/><FieldRef Name="LSB_Conflict"/><FieldRef Name="Document_x0020_Type"/><FieldRef Name="LSB_AuthorNames"/><FieldRef Name="Year"/><FieldRef Name="LSB_LegalFileNumber"/><FieldRef Name="RoutingRuleDescription"/><FieldRef Name="LevelOfCourt"/></ViewFields>';
  ...
  public lookupLibByUrlFragment: Map<string, string> = new Map<
    string,
    string
  >();
  public lookupUrlFragmentByLibName: Map<string, string> = new Map<
    string,
    string
  >();
  ...
  public componentDidMount(): void {
    ...
    this._spService
      .GetLibDropdownOptions(
        this.lookupLibs,
        this.topicIdLookup,
        this.lookupLibByUrlFragment,
        this.lookupUrlFragmentByLibName,
        this.libsList,
  ...
  public getSelectedListTitleArea1 = (ev: any, data: any) => {
    ...
    this._spService
      .getArea1Topics(this.topicLookup, this.topicIdLookup, data.text)
      .then((resultTopic: any) => {
        this.setState({ listAreaTopics1: resultTopic });
      });
    this.setState({ searchEnabled: true, firstTopicEnabled: true, secondAreaEnabled: true });
  };
  ...
  public getSelectedListTitleArea2 = (ev: any, data: any) => {
    ...
    this._spService
      .getArea2Topics(this.topicLookup, this.topicIdLookup, data.text)
      .then((resultTopic: any) => {
  ...
  public getQuerySixFilters = (queries: string[]): string => {
    ...
    var query =
      ...
      cond5 +
      cond6 +
      "</And></And></And></And></And></Where></Query>" +
      this.viewfields +
      "</View>";
    return query;
  };
  ...
  public getQuerySevenFilters = (queries: string[]): string => {
    ...
    var query =
      ...
      cond6 +
      cond7 +
      "</And></And></And></And></And></And></Where></Query>" +
      this.viewfields +
      "</View>";
    return query;
  };
  ...
  public getQueryEightFilters = (queries: string[]): string => {
    ...
    var query =
      ...
      cond7 +
      cond8 +
      "</And></And></And></And></And></And></And></Where></Query>" +
      this.viewfields +
      "</View>";
    return query;
  };
  ...
  public getQueryNineFilters = (queries: string[]): string => {
    ...
    var query =
      ...
      cond8 +
      cond9 +
      "</And></And></And></And></And></And></And></And></Where></Query>" +
      this.viewfields +
      "</View>";
    return query;
  };
  ...
  public SearchDoc(
    ...
    legislationLookup: Map<number, string>,
    docTypeLookup: Map<number, string>,
    yearLookup: Map<number, string>
  ) {
    ...
    let libSelect: string = document
      .getElementById("drLib-input")
      .getAttribute("value");
    this.finalCount = 0;
    ...
    if (this.keyPhraseValue !== undefined) {
      ...
      let lastChar = this.keyPhraseValue.substring(
        keyPhraseLength - 1,
        keyPhraseLength
      );
      if (
    ...
    console.log(
      "Filter count: " +
        filterCount +
        " (including Approval Status = 'Approved')"
    );

    switch (n) {
      case 1:
        finalQuery = this.getQueryOneFilter(queries);
        break;
      case 2:
        finalQuery = this.getQueryTwoFilters(queries);
        break;
      case 3:
        finalQuery = this.getQueryThreeFilters(queries);
        break;
      case 4:
        finalQuery = this.getQueryFourFilters(queries);
        break;
      case 5:
        finalQuery = this.getQueryFiveFilters(queries);
        break;
      case 6:
        finalQuery = this.getQuerySixFilters(queries);
        break;
      case 7:
        finalQuery = this.getQuerySevenFilters(queries);
        break;
      case 8:
        finalQuery = this.getQueryEightFilters(queries);
        break;
      case 9:
        finalQuery = this.getQueryNineFilters(queries);
        break;
      default:
    }
      ...
      sp.web.lists
        ...
        .then((results: any) => {
          results.map((result: any) => {
            ...
            let top = this._joinLookupArrayValues(
              result.LSB_TopicsOfLawId,
              topicLookup,
              "; "
            );
            if (!top)
              top = this._joinLookupArrayValues(
                result.LSB_TopicsOfLaw2Id,
                topicLookup,
                "; "
              );
            if (!top) top = "";
            if (selTopic1)
              topFound = this.containsVal(top, this.selectedListTitleTopic1);
            let ar2 = areaLookup.get(result.LSB_AreaOfLaw2Id);
            if (!ar2) ar2 = "";
            let to2Found: boolean = false;
            let to2 = this._joinLookupArrayValues(
              result.LSB_TopicsOfLaw2Id,
              topicLookup,
              "; "
            );
            if (!to2)
              to2 = this._joinLookupArrayValues(
                result.LSB_TopicsOfLawId,
                topicLookup,
                "; "
              );
            if (!to2) to2 = "";
            if (selTopic2)
              to2Found = this.containsVal(to2, this.selectedListTitleTopic2);

            if (selTopic1 || selTopic2 || topFound || to2Found) keep = true;
            else if (are !== undefined) keep = true;

            if (keep) {
              let leg = this._joinLookupArrayValues(
                result.LSB_LegislationId,
                legislationLookup,
                "; "
              );
              if (!leg) leg = "";
              let aut = this._joinLookupArrayValues(
                result.LSB_AuthorNamesId,
                this.authorLookup,
                "; "
              );
              if (!aut) aut = "";
              ...
              try {
                ...
                let lbu =
                  this.listRootUrl + this.lookupUrlFragmentByLibName.get(lib);
                let yea = result.Year;
                if (!yea) yea = "";
          ...
          this._getPage(1);
          document.getElementById("dv_SearchResults").style.display =
            "table-cell";
          document.getElementById("dv_tableCaption").style.display = "block";
          document.getElementById("dv_Table").style.display = "block";
          if (this.finalCount > this.pageSize)
            document.getElementById("dv_pagination").style.display = "block";
        });
    }
  }
  ...
  private _joinLookupArrayValues(
    theArray: number[],
    theLookup: Map<number, string>,
    joiner: string
  ) {
    let joined: string = "";
    let i = 0;
  ...
  public render(): React.ReactElement<ISearchDocProps> {
    ...
    return (
      <section
        className={`${styles.searchDoc} ${hasTeamsContext ? styles.teams : ""}`}
      >
        <div className={styles.dv_ParentDic}>
          <div className={styles.inputBlock}>
            ...
            <ComboBox
              id="drpArea1"
              options={this.state.listTitlesArea1}
              autoComplete="on"
              required={false}
              placeholder={"Select Area of Law (1)"}
              onChange={this.getSelectedListTitleArea1}
            />
          </div>
          <div className={styles.inputBlock}>
            ...
            <ComboBox
              id="drTopic1"
              options={this.state.listAreaTopics1}
              autoComplete="on"
              disabled={!this.state.firstTopicEnabled}
              placeholder={"Select Topics Of Law (1)"}
              onChange={this.getselectedListTitleTopic1}
            />
          </div>
          <div className={styles.inputBlock}>
            ...
            <ComboBox
              id="drpArea2"
              options={this.state.listTitlesArea2}
              autoComplete="on"
              disabled={!this.state.secondAreaEnabled}
              placeholder={"Select Area of Law (2)"}
              onChange={this.getSelectedListTitleArea2}
            />
          </div>
          <div className={styles.inputBlock}>
            ...
            <ComboBox
              id="drTopic2"
              options={this.state.listAreaTopics2}
              autoComplete="on"
              disabled={!this.state.secondTopicEnabled}
              placeholder={"Select Topics Of Law (2)"}
              onChange={this.getselectedListTitleTopic2}
            />
          </div>
          <div className={styles.inputBlock}>
            ...
            <ComboBox
              id="drLegislation"
              options={this.state.listTitlesLegislation}
              autoComplete="on"
              placeholder={"Select Legislation"}
              onChange={this.getselectedListTitleLegislation}
            />
          </div>
          <div className={styles.inputBlock}>
            ...
            <ComboBox
              id="drDocType"
              options={this.state.listTitlesDocType}
              autoComplete="on"
              placeholder={"Select Document Type"}
              onChange={this.getSelectedListTitleDocType}
            />
          </div>
          <div className={styles.inputBlock}>
            ...
            <ComboBox
              id="drAuthor"
              options={this.state.listTitlesAuthor}
              autoComplete="on"
              placeholder={"Select Author"}
              onChange={this.getSelectedListTitleAuthor}
            />
          </div>
          <div className={styles.inputBlock}>
            ...
            <ComboBox
              id="drYear"
              options={this.state.listTitlesYear}
              autoComplete="on"
              placeholder={"Select Year"}
              onChange={this.getSelectedListTitleYear}
            />
          </div>
          <div className={styles.inputBlock}>
            ...
            <ComboBox
              id="drConflict"
              options={this.conflict}
              autoComplete="on"
              placeholder={"Select Yes or No"}
              onChange={this.getSelectedConflict}
            />
          </div>
          <div className={styles.inputBlock}>
            ...
            <ComboBox
              id="drLevelOfCourt"
              options={this.state.listTitlesLevelOfCourt}
              autoComplete="on"
              placeholder={"Select Level of Court"}
              onChange={this.getSelectedLevelOfCourt}
            />
          </div>
          <div className={styles.inputBlock}>
            ...
            <ComboBox
              id="drLib"
              options={this.state.listTitlesLibrary}
              selectedKey={0}
              placeholder={"Select Library"}
              onChange={this.getSelectedLibrary}
            />
          </div>
          <div className={styles.inputBlock}>
          ...
          <div className={styles.inputBlock}>
            ...
            <div className={styles.descStyle}>
              <input
                id="keywordPhrase"
                name="keywordPhrase"
                className="keywordPhrase"
                autoComplete="on"
                type="text"
                placeholder={"Enter keywords or phrase (in quotes)"}
                onDoubleClick={this._onKeywordDoubleClick}
                onChange={this._onKeyPhraseChange}
              />
            </div>
          </div>
          <div className={styles.divButton}>
            ...
            <PrimaryButton
              ...
              disabled={!this.state.searchEnabled}
              text="Clear"
              alt="Clear form by refreshing page"
        ...
        <div id="dv_resultsContainer">
          <div id="dv_Table">
            <div id="dv_tableCaption" className={styles.tableCaptionStyle}>
              Document Search Results ({this.finalCount} &quot;
              {this.statusFilter}&quot; documents found)
            </div>
                  ...
                  <>
                    <div className={styles.CardStyle} key={key}>
                      ...
                      <div className={styles.DocPropStyle}>
                        ...
                        {
                          // if the library is not Labour and Employment, render normally
                          item.LSB_LibURL.indexOf("LabourEmployment") === -1 ? (
                            <a href={item.LSB_LibURL} target="_blank">
                              {item.LSB_LibName}
                            </a>
                          ) : (
                            ...
                            <span className="LandEHighlight">
                              ...
                              >
                                <a
                                  href={item.LSB_LibURL}
                                  target="_blank"
                                  className="LandEHighlight"
                                  style={{
                                    color: "red",
                                    cursor: "pointer",
                                    fontSize: 11,
                      ...
                      <div className={styles.DocPropStyle}>
                        <strong>Area of Law (1):</strong> {item.LSB_AreaOfLaw1}
                      </div>
                      <div className={styles.DocPropStyle}>
                        <strong>Topics (1):</strong> {item.LSB_TopicsOfLaw1}
                      </div>
                      <div className={styles.DocPropStyle}>
                        <strong>Area of Law (2):</strong> {item.LSB_AreaOfLaw2}
                      </div>
                      <div className={styles.DocPropStyle}>
                        <strong>Topics (2):</strong> {item.LSB_TopicsOfLaw2}
                      </div>
                      <div className={styles.DocPropStyle}>
                        <strong>Legislation:</strong> {item.LSB_Legislation}
                      </div>
                      <div className={styles.DocPropStyle}>
                        <strong>Restrictions:</strong> {item.LSB_Conflict}
                      </div>
                      <div className={styles.DocPropStyle}>
                        <strong>Level of Court:</strong> {item.LevelOfCourt}
                      </div>
                      <div className={styles.DocPropStyle}>
                        <strong>Year:</strong> {item.Year}
                      </div>
                      <div className={styles.DocPropStyle}>
                        <strong>Doc Type:</strong> {item.Document_x0020_Type}
                      </div>
                      <div className={styles.DocPropStyle}>
                        <strong>Author(s):</strong> {item.LSB_AuthorNames}
                      </div>
                      <div className={styles.DocPropStyle}>
                        <strong>Description:</strong>{" "}
                        <i>{item.RoutingRuleDescription}</i>
                      </div>
                    </div>
                  </>
                );
  ...
  private _getPage(page: number) {
    ...
    this.setState({
      paginatedItems: this.state.items.slice(
        roundUpPage * this.pageSize,
        roundUpPage * this.pageSize + this.pageSize
      ),
    });
  }
}
